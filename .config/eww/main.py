...
icon_default = "/home/Maximus/.local/share/notif/notif.png"
        ...
        string = string + f"""
                  (button :class 'notif'
                   (box :orientation 'horizontal' :space-evenly false
                      (image :class "notif-image" :image-width 24 :image-height 24 :path '{item.icon or ''}')
                      (box :orientation 'vertical'
                        (label :width 150 :wrap true :text '{item.summary or ''}')
                        (label :class "notif-text" :width 150 :wrap true :text '{item.body or ''}')
                  )))
                  """
